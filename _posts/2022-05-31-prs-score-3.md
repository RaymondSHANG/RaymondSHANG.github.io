...
{{tip}} <b> Key point to calculate PRS </b> <br/><br/>The formula to calculate PRS score is given in "PRS score 2" blog, where 'a hyperparameter'(pvalue threshold) that determine the set of SNPs to be included when calculating PRS scores.

$$
    PRS_j =\frac{ \sum_i^NS_i*G_{ij}}{P*M_j}
$$

<br/><br/>Where the effect size of SNP $i$ is $S_i$; the number of effect alleles observed in sample $j$ is $G_{ij}$; the ploidy of the sample is $P$ (is genrally 2 for human); <b>the total number of SNPs</b> included in the PRS is N;and the number of non-missing SNPs observed in sample $j$ is $M_j$. If the sample has a missing genotype for SNP $i$, then the population minor allele frequency multiplied by the ploidy ($MAF_i * P$) is used instead of $G_{ij}$.
